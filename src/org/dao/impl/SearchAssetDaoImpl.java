...
public class SearchAssetDaoImpl implements SearchAssetDao {
    @Override
    public List<Asset> getAsset(Integer id) {
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT t1.*,t2.tid,t2.id,t2.state,t3.id,t3.voucherno,t3.datetime,t4.name FROM product t1 INNER JOIN asset t2 ON t1.id=t2.pid INNER JOIN finance t3 on t2.id=t3.aid INNER JOIN account t4 ON t3.uid=t4.id where t2.id=?");
            ps.setInt(1, id);
            rs = ps.executeQuery();
